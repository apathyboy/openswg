...
namespace gsCore
{
     ...
     */
    class GS_CORE_EXPORT IEventManager
    {
    public:
        enum eConstants
        {
         ...
         */
        virtual bool tick(uint64 deltaMilliseconds = kINFINITE) = 0;
    ...
    private:
        ...
        // mechanics.
        friend bool GS_CORE_EXPORT safeAddListener(EventListenerPtr const & inHandler,
                                    EventType const & inType);

        friend bool GS_CORE_EXPORT safeDelListener(EventListenerPtr const & inHandler,
                                    EventType const & inType);

        friend bool GS_CORE_EXPORT safeTriggerEvent(Event const & inEvent);

        friend bool GS_CORE_EXPORT safeQueueEvent(EventPtr const & inEvent);

        friend bool GS_CORE_EXPORT safeAbortEvent(EventType const & inType,
                                   bool allOfType = false);

        friend bool GS_CORE_EXPORT safeTickEventManager(unsigned long maxMillis = 
                                    IEventManager::kINFINITE);

        friend bool GS_CORE_EXPORT safeValidateEventType(EventType const & inType);
    };

    typedef GS_CORE_EXPORT std::vector<EventListenerPtr> EventListenerList;
    typedef GS_CORE_EXPORT std::vector<EventType>        EventTypeList;

    /** EventManager is a basic implementation of the event listener registry
     *  defined by IEventManager.
     */
    class GS_CORE_EXPORT EventManager : public IEventManager
    {
    public:
         ...
         *  completed, false otherwise (e.g. timeout)
         */
        bool tick(uint64 deltaMilliseconds);
    ...
    bool GS_CORE_EXPORT safeAddListener(EventListenerPtr const & inHandler,
                         EventType const & inType);

    bool GS_CORE_EXPORT safeDelListener(EventListenerPtr const & inHandler,
                         EventType const & inType);

    bool GS_CORE_EXPORT safeTriggerEvent(Event const & inEvent);

    bool GS_CORE_EXPORT safeQueueEvent(EventPtr const & inEvent);

    bool GS_CORE_EXPORT safeAbortEvent(EventType const & inType,
                        bool allOfType);

    bool GS_CORE_EXPORT safeTickEventManager(unsigned long maxMillis);

    bool GS_CORE_EXPORT safeValidateEventType(EventType const & inType);
}
