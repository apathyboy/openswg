...
{
	class OS_SOEPROTOCOL_EXPORT Creo1Message : public gsNetwork::NetworkMessage
	{
	public:
		uint32 healthModifiers;
		uint32 strengthModifiers;
		uint32 constitutionModifiers;
		uint32 actionModifiers;
		uint32 quicknessModifiers;
		uint32 staminaModifiers;
		uint32 mindModifiers;
		uint32 focusModifiers;
		uint32 willpowerModifiers;

		uint64 objectId;
		uint32 bankCredits;
		uint32 inventoryCredits;
