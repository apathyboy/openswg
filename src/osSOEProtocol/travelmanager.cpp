...
void TravelManager::updateTransports()
{
	if (! m_transportsLanded)
	{
		...
		{
			uint64 objectId = (*itor)->getPropertyAs<Uint64ObjectProperty*>(std::string("ObjectId"))->getValue();
			uint8 posture = 0;
...
void TravelManager::update(const uint64 updateTimestamp)
{
...
void TravelManager::loadTransports()
{
	...
	try
    {
		...
		if (result)
        {
            ...
            {		
				...
				transport->getPropertyAs<Uint64ObjectProperty*>(std::string("DatabaseId"))->setValue((uint64)row["id"]);
				transport->getPropertyAs<Uint64ObjectProperty*>(std::string("ObjectId"))->setValue((uint64)row["object_id"]);
				transport->getPropertyAs<Uint64ObjectProperty*>(std::string("ParentId"))->setValue((uint64)row["cell_id"]);
				...
				transport->getPropertyAs<UintObjectProperty*>(std::string("State"))->setValue((uint)row["state"]);
				++count;
	...
	catch (const mysqlpp::BadQuery& er)
    {
		...
		return;
			...
			Log::getMainLog().debug("Loaded %i transports.", count);
		// Continue normally.
	}
...
void TravelManager::loadTicketCollectors()
{
	...
	try
    {
		...
		if (result)
        {
            ...
            {		
				...
				collector->getPropertyAs<Uint64ObjectProperty*>(std::string("DatabaseId"))->setValue((uint64)row["id"]);
				collector->getPropertyAs<Uint64ObjectProperty*>(std::string("ObjectId"))->setValue((uint64)row["object_id"]);
				collector->getPropertyAs<Uint64ObjectProperty*>(std::string("ParentId"))->setValue((uint64)row["cell_id"]);
				...
				collector->getPropertyAs<UintObjectProperty*>(std::string("State"))->setValue((uint)row["state"]);
				++count;
        ...
        {
            Log::getMainLog().error("Failed to load ticket collectors.");
        }
    }
	...
	catch (const mysqlpp::BadQuery& er)
    {
		...
		return;
			...
			Log::getMainLog().debug("Loaded %i ticket collectors.", count);
		// Continue normally.
	}
	catch (const mysqlpp::Exception& er)
    {
...
void TravelManager::loadTicketVendors()
{
	...
	try
    {
		...
		if (result)
        {
            ...
            {		
				...
				vendor->getPropertyAs<Uint64ObjectProperty*>(std::string("DatabaseId"))->setValue((uint64)row["id"]);
				vendor->getPropertyAs<Uint64ObjectProperty*>(std::string("ObjectId"))->setValue((uint64)row["object_id"]);
				vendor->getPropertyAs<Uint64ObjectProperty*>(std::string("ParentId"))->setValue((uint64)row["cell_id"]);
				...
				vendor->getPropertyAs<UintObjectProperty*>(std::string("State"))->setValue((uint)row["state"]);
				++count;
        ...
        {
            Log::getMainLog().error("Failed to load ticket vendors.");
        }
    }
	...
	catch (const mysqlpp::BadQuery& er)
    {
		...
		return;
			...
			Log::getMainLog().debug("Loaded %i ticket vendors.", count);
		// Continue normally.
	}
	catch (const mysqlpp::Exception& er)
    {
