...
ServerEventListener::ServerEventListener(ServerApplication* server)
: m_server(server)
{}
...
void ServerEventListener::registerEvents(EventListenerPtr listener)
{
	safeAddListener(listener, EventType(Event_SessionCreated::gkName));
}

bool ServerEventListener::handleEvent(const Event &event)
{
	return false;
}
